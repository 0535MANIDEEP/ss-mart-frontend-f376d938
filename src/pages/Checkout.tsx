...
const schema = yup.object().shape({
  ...
  phone: yup.string().required("Phone is required").matches(/^[6-9]\d{9}$/, "Invalid Indian mobile number"),
  address: yup.string().required("Address required").min(8),
});
    ...
    <div className="max-w-lg mx-auto mt-10 bg-white rounded shadow p-7 animate-fade-in">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="text-gray-500 text-xs mt-2 text-center">
          Your order will open WhatsApp chat. No payment required now.
        </div>
      </form>
    </div>
  );
